...
/**
 * Workspace paths served by the plugin, in order:
 *   1. the in-memory DSH `workspaceRegistry.list()` when reachable, and
 *   2. the persisted registry file `$DSH_HOME/storages/workspace.json`
 *      (`tables.workspaces[*].path`), which is always current on disk and does
 *      not depend on the service being reachable from this plugin's scope.
 * Combined and de-duplicated; each request reads fresh so newly created
 * workspaces are served immediately after they are persisted.
 */
async function workspaceRoots(ctx, env) {
  const paths = []
  // In-memory registry (best-effort).
  try {
  ...
  } catch { /* ignore — fall through to persisted file */ }
  // Persisted registry file (authoritative + always current).
  try {
    const home = (env?.DSH_HOME && env.DSH_HOME.trim() !== '') ? env.DSH_HOME : join(os.homedir(), '.dsh')
    ...
    if (table && typeof table === 'object') {
      for (const record of Object.values(table)) {
        if (record && typeof record.path === 'string' && record.path.trim() !== '') paths.push(resolve(record.path))
      }
    }
  } catch { /* no persisted registry yet */ }
  return unique(paths)
}
...
function apply(ctx, config) {
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const roots = unique([...explicitRoots(config, process.env), ...await workspaceRoots(ctx, process.env)])
